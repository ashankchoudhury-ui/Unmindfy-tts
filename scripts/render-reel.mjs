...
async function run(cmd, args) {
  return new Promise((resolve, reject) => {
    ...
    p.on('error', reject);
    p.on('close', c => c ? reject(new Error(`${cmd} exited ${c}\n${e.slice(-20000)}`)) : resolve({ stdout: o, stderr: e }));
  });
}

async function probe(file) {
  const r = await run('ffprobe', ['-v', 'error', '-show_entries', 'format=duration,size:stream=index,codec_type,codec_name,width,height,pix_fmt,bit_rate,avg_frame_rate,r_frame_rate,profile,level,sample_rate,channels,channel_layout', '-of', 'json', file]);
  ...
  try {
    ...
    while (offset < size) {
      ...
      for (let attempt = 1; attempt <= 3; attempt++) {
        try {
          response = await fetch(uploadUrl, { method: 'PUT', headers: { 'Content-Length': String(length), 'Content-Range': `bytes ${offset}-${offset + length - 1}/${size}`, 'Content-Type': 'video/mp4' }, body: buffer });
...
function layoutWords(words) {
  ...
  for (const w of words) {
    ...
    if (line.length && need > MAX) { lines.push(line); line = [w]; n = w.length; }
...
function validateTimings(timings, script) {
  ...
  for (let i = 0; i < timings.length; i++) {
    ...
    if (!Number.isFinite(s) || !Number.isFinite(e) || s < 0 || e <= s) throw new Error(`Invalid word timing at ${i + 1}: ${JSON.stringify(timings[i])}`);
    if (i > 0 && s <= prev) throw new Error(`Non-increasing real word timing at ${i + 1}: ${s} <= ${prev}`);
    prev = s;
  }
  return timings;
}
...
async function makeAss(script, timings, out) {
  ...
  const words = wordsOf(script), pages = captionPages(words);
  const FONT = 56, BOX_W = 780, SAFE_X = (OUTPUT_WIDTH - BOX_W) / 2, CAPTION_Y = 96;
  const ass = [
    '[Script Info]', 'ScriptType: v4.00+', `PlayResX: ${OUTPUT_WIDTH}`, `PlayResY: ${OUTPUT_HEIGHT}`,
  ...
  for (const page of pages) {
    const first = timings[cursor], last = timings[cursor + page.length - 1];
    if (!first || !last) throw new Error('Caption page timing lookup failed');
    const eventStart = first.start, eventEnd = Math.max(last.end + 0.28, eventStart + 0.45);
...
function fpsNumber(rate) {
  ...
  return Number.isFinite(a) && Number.isFinite(b) && b ? a / b : Number(rate) || 0;
}

function sourceScore(file) {
  const w = Number(file.videoMediaMetadata?.width) || 0;
  const h = Number(file.videoMediaMetadata?.height) || 0;
  if (w < OUTPUT_WIDTH || h < OUTPUT_HEIGHT) return -Infinity;
  const aspectPenalty = Math.abs(Math.log((w / h) / TARGET_ASPECT));
  return 1e15 - aspectPenalty * 1e12 + w * h;
}

async function renderFinal(footage, ass, audio, total, out) {
  const src = (await probe(footage)).streams?.find(s => s.codec_type === 'video') || {};
  const sw = Number(src.width) || 0, sh = Number(src.height) || 0;
  const sourceFps = fpsNumber(src.avg_frame_rate || src.r_frame_rate);
  if (sw < OUTPUT_WIDTH || sh < OUTPUT_HEIGHT) throw new Error(`Source footage is below final resolution: ${sw}x${sh}. Need at least ${OUTPUT_WIDTH}x${OUTPUT_HEIGHT} for 16:9 output.`);
  if (!sourceFps || sourceFps < 15 || sourceFps > 120) throw new Error(`Unsupported source frame rate: ${src.avg_frame_rate || src.r_frame_rate}`);
  ...
  await run('ffmpeg', [
    ...
    '-map', '0:v:0', '-map', '1:a:0',
    '-c:v', 'libx264', '-preset', 'veryslow', '-crf', '10', '-profile:v', 'high',
...
async function verify(file, expected, expectedFps) {
  const st = await fs.stat(file);
  if (st.size < 1e6) throw new Error(`Rendered video is suspiciously small: ${st.size}`);
  const d = await probe(file), v = d.streams?.find(s => s.codec_type === 'video'), a = d.streams?.find(s => s.codec_type === 'audio');
  if (!v || !a) throw new Error('Final file is missing video or audio');
  if (Math.abs(Number(d.format?.duration || 0) - expected) > 1) throw new Error('Final duration mismatch');
  if (Number(v.width) !== OUTPUT_WIDTH || Number(v.height) !== OUTPUT_HEIGHT) throw new Error(`Unexpected final video size: ${v.width}x${v.height}; expected ${OUTPUT_WIDTH}x${OUTPUT_HEIGHT}`);
  if (Math.abs((Number(v.width) / Number(v.height)) - TARGET_ASPECT) > 0.001) throw new Error('Final video is not 16:9');
  if (v.pix_fmt !== 'yuv420p' || String(v.codec_name) !== 'h264') throw new Error(`Unexpected final video format: ${v.codec_name}/${v.pix_fmt}`);
  const actualFps = fpsNumber(v.avg_frame_rate || v.r_frame_rate);
  if (!actualFps || Math.abs(actualFps - expectedFps) > 0.05) throw new Error(`Frame rate changed unexpectedly: source=${expectedFps} output=${actualFps}`);
  if (Number(v.bit_rate || 0) < 5e6) throw new Error(`Final video bitrate is too low: ${v.bit_rate}`);
  if (String(a.codec_name) !== 'aac' || Number(a.sample_rate) !== 44100 || Number(a.channels) !== 2) throw new Error('Unexpected final audio format');
...
async function main() {
  ...
  const jobData = await api('/api/edit/job');
  const job = jobData?.job;
  if (!job) { console.log('No render job available.'); return; }
  console.log(`Rendering ${job.id}: ${job.reel || 'untitled'}`);
  ...
  try {
    ...
    const [footageFiles, musicFiles] = await Promise.all([folderDescendants(token, footageFolder.id), folderDescendants(token, musicFolder.id)]);

    const configuredFootage = job.render_footage_file_id ? footageFiles.find(f => f.id === job.render_footage_file_id) : null;
    const eligibleFootage = footageFiles.filter(f => (FOOTAGE_EXT.test(f.name) || String(f.mimeType || '').startsWith('video/')) && !GENERATED_RE.test(f.name));
    const footage = configuredFootage || [...eligibleFootage].sort((a, b) => sourceScore(b) - sourceScore(a) || Number(b.size || 0) - Number(a.size || 0))[0];
    if (!footage) throw new Error('No eligible source footage found in UNMINDY/Footage');

    const configuredMusic = job.render_music_file_id ? musicFiles.find(f => f.id === job.render_music_file_id) : null;
    const eligibleMusic = musicFiles.filter(f => AUDIO_EXT.test(f.name) && !GENERATED_RE.test(f.name));
    const music = configuredMusic || eligibleMusic[0];
    if (!music) throw new Error('No music file found in UNMINDY/Music');

    console.log(`Source: ${footage.name}`);
    console.log(`Music: ${music.name}`);

    const footagePath = path.join(WORK, `footage${path.extname(footage.name) || '.mp4'}`);
    const musicPath = path.join(WORK, `music${path.extname(music.name) || '.mp3'}`);
    const voicePath = path.join(WORK, 'voice.wav');
    const ambientPath = path.join(WORK, 'ambient.m4a');
    ...
    await downloadDriveFile(token, { id: job.tts_audio_url.match(/[-\w]{20,}/)?.[0] || '', name: 'voice' }, voicePath).catch(async () => {
      const r = await fetch(job.tts_audio_url);
      if (!r.ok || !r.body) throw new Error(`TTS audio download failed: ${r.status}`);
      await pipeline(r.body, fsSync.createWriteStream(voicePath));
    });

    const total = Number(await (async () => { const r = await run('ffprobe', ['-v', 'error', '-show_entries', 'format=duration', '-of', 'default=nw=1:nk=1', voicePath]); return r.stdout.trim(); })());
    if (!Number.isFinite(total) || total <= 0) throw new Error('Invalid TTS audio duration');
    ...
    const { sourceFps } = await renderFinal(footagePath, assPath, audioPath, total, OUT);
    const result = await verify(OUT, total, sourceFps);
    console.log(`Verified ${result.video.width}x${result.video.height} @ ${fpsNumber(result.video.avg_frame_rate || result.video.r_frame_rate).toFixed(3)}fps, ${(result.size / 1048576).toFixed(1)} MiB`);

    const uploaded = await uploadDriveFile(token, OUT, `reel-${job.id}-reference-style-final.mp4`, exportsFolder.id);
    await api('/api/edit/complete', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: job.id, status: 'Ready', export_drive_file_id: uploaded.id, export_drive_url: uploaded.webViewLink || null }) });
    console.log(`Exported: ${uploaded.webViewLink || uploaded.id}`);
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    console.error(message);
    await api('/api/edit/complete', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: job.id, status: 'Failed', edit_error: message }) }).catch(e => console.error('Failed to report render error:', e));
    process.exitCode = 1;
  }
}
