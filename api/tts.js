...
async function uploadAudio(path, wav) {
  ...
  const response = await fetch(`${SUPABASE_URL}/storage/v1/object/${TTS_BUCKET}/${path}`, {
    method: 'POST', headers: { apikey: key, Authorization: `Bearer ${key}`, 'Content-Type': 'audio/wav', 'x-upsert': 'true' }, body: wav
  });
...
async function getRecord(recordId) {
  const rows = await supabaseRequest(`content_pipeline?id=eq.${encodeURIComponent(recordId)}&select=id,script,tts_status,tts_audio_url&limit=1`);
  return rows[0] || null;
}
...
function extractGeminiAudio(interaction) {
  if (interaction?.output_audio?.data) return { data: interaction.output_audio.data, sampleRate: interaction.output_audio.sample_rate || 24000, channels: interaction.output_audio.channels || 1, mimeType: interaction.output_audio.mime_type || 'audio/l16' };
  const steps = Array.isArray(interaction?.steps) ? interaction.steps : [];
  for (let i = steps.length - 1; i >= 0; i -= 1) {
    const content = Array.isArray(steps[i]?.content) ? steps[i].content : [];
    for (let j = content.length - 1; j >= 0; j -= 1) {
      const item = content[j];
      if (item?.type === 'audio' && item?.data) return { data: item.data, sampleRate: item.sample_rate || 24000, channels: item.channels || 1, mimeType: item.mime_type || 'audio/l16' };
    }
  }
  return null;
}
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!recordId || !/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(recordId)) return json(res, 400, { error: 'A valid Supabase UUID recordId is required' });
    const force = req.body?.force === true;
    const record = await getRecord(recordId);
    if (!record) return json(res, 404, { ok: false, error: 'Content pipeline record not found' });
    if (record.tts_status === 'Ready' && record.tts_audio_url && !force) return json(res, 200, { ok: true, recordId, audioUrl: record.tts_audio_url, model: MODEL, alreadyReady: true });
    ...
    const geminiResponse = await fetch(GEMINI_URL, {
      method: 'POST', headers: { 'x-goog-api-key': env('GEMINI_API_KEY'), 'Content-Type': 'application/json', 'Api-Revision': '2026-05-20' },
      body: JSON.stringify({ model: MODEL, input: getVoicePrompt(cleanScript), response_format: { type: 'audio' }, generation_config: { speech_config: [{ voice: 'Algieba' }] } })
    });
    ...
    if (recordId) { try { await updateRecord(recordId, { tts_status: 'Error', tts_started_at: null }); } catch {} }
