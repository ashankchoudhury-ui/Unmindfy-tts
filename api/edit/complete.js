...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { id, status, export_drive_file_id, export_drive_url, error } = req.body || {};
    ...
    const response = await fetch(`${SUPABASE_URL}/rest/v1/content_pipeline?id=eq.${encodeURIComponent(id)}`, {
      method: 'PATCH',
      headers: {
        apikey: key,
      ...
      body: JSON.stringify({
        edit_status: status,
        export_drive_file_id: export_drive_file_id || null,
        export_drive_url: export_drive_url || null,
        edit_error: status === 'Failed' ? String(error || 'Unknown rendering error').slice(0, 4000) : null,
