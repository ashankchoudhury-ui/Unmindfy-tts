...
function eligible(record, staleBeforeMs) {
  ...
  if (record.tts_status === 'Generating') {
    if (record.id === ONE_TIME_REGEN_ID && (attempts === 2 || attempts === 3)) return true;
...
async function claim(record, staleBeforeMs) {
  ...
  if (record.id === ONE_TIME_REGEN_ID && (record.tts_status === 'Ready' || (record.tts_status === 'Generating' && (attempts === 2 || attempts === 3)))) {
